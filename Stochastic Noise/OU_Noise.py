...
# Parameters
params1 = [3.25, 100, 22.0, 50.0, 20.0, 500.0, 135.0, 220.0, 0.56, 6.0, 2.5, 1, 10**(-0.5), 100] + [10 * np.random.rand() for _ in range(11)]

X = np.array(params1[15:]).reshape(-1, 1)  # Initial state
OU = params1[14]
...
# Extract parameters
A, a, B, b, G, g, Cvec, P, r, v0, e0, ss, tau, D = params1[:14]

params = {
    'A': A, 'a': a, 'B': B, 'b': b, 'G': G, 'g': g, 'Cvec': Cvec,
    'P': P, 'r': r, 'v0': v0, 'e0': e0, 'ss': ss, 'tau': tau, 'D': D
}
...
for j in range(len(t)):
    OU = OU - (OU / params['tau']) * dt + (np.sqrt(2 * params['D']) / params['tau']) * dW[0, j]
    ...
    X[6] = X[6] + (Aa * (params['P'] + C2 * 2 * params['e0'] / (1 + np.exp(params['r'] * (params['v0'] - (C1 * X[0]))))) - 2 * params['a'] * X[6] - X[1] * a2) * dt + Aa * params['ss'] * OU
